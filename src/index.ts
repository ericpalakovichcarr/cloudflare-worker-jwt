...
class JWT {
    ...
    protected _decodePayload(raw: string): JWTPayload | null {
        switch (raw.length % 4) {
            case 0:
                break
            case 2:
    ...
    public async sign(payload: JWTPayload, secret: string, algorithm: 'HS256' | 'HS512' = 'HS256') {
    ...
    public async verify(token: string, secret: string, algorithm: 'HS256' | 'HS512' = 'HS256') {
